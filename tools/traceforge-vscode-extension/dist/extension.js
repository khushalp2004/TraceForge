...
const writeTextFile = async (uri, text) => {
...
const appendEnvVar = async (envUri, key, value) => {
    let existing = "";
    try {
...
const findPackageJsons = async () => vscode.workspace.findFiles("**/package.json", "**/node_modules/**");
...
const pickWorkspaceFolder = async () => {
    ...
    if (folders.length === 1)
        return folders[0];
    const pick = await vscode.window.showQuickPick(folders.map((f) => ({ label: f.name, folder: f })), { placeHolder: "Select workspace folder" });
    return pick?.folder ?? null;
};
const findCandidates = async (root) => {
    const pkgs = await findPackageJsons();
    const entries = [];
    for (const uri of pkgs) {
        const pkg = await readJsonFile(uri);
        if (!pkg)
            continue;
...
const chooseProjectFolder = async (kind, entries) => {
    const filtered = entries.filter((e) => (kind === "backend" ? e.isBackend : e.isFrontend));
    if (filtered.length === 0)
        return null;
    if (filtered.length === 1)
        return filtered[0];
    const pick = await vscode.window.showQuickPick(filtered.map((e) => ({
        label: e.rel || ".",
...
const patchBackendEntrypoint = async (entryUri, backendRoot) => {
    ...
    if (!text.includes("installTraceForge")) {
        ...
        const appMatch = text.match(/const\s+(\w+)\s*=\s*express\s*\(\s*\)\s*;?/m) ||
        ...
        const patchedImports = text.slice(0, insertAt) +
            dotenvLine +
            importLine +
            text.slice(insertAt);
        ...
        edit.replace(entryUri, new vscode.Range(doc.positionAt(0), doc.positionAt(doc.getText().length)), finalText);
...
`;
    ...
    for (const rel of candidates) {
        ...
        const text = doc.getText();
        if (text.includes("initTraceForgeBrowser"))
            return;
...
const activate = (context) => {
    context.subscriptions.push(vscode.commands.registerCommand("traceforge.configure", async () => {
        const ingestUrl = await vscode.window.showInputBox({
            title: "TraceForge ingest URL",
            value: vscode.workspace.getConfiguration("traceforge").get("ingestUrl") || "http://localhost:3001/ingest"
        });
        if (!ingestUrl)
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand("traceforge.initialize", async () => {
        ...
        if (!patched) {
            vscode.window.showWarningMessage("Backend initialized, but entrypoint patch was not applied. Add installTraceForge(app) before app.listen().");
        }
        if (frontend) {
